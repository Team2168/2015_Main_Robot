...
public class F310 extends Joystick {
    ...
    private double falconClaw(double inputSpeed, double brake) {
        ...
        // return ((1 - ((-RobotMap.minDriveSpeed.getDouble() + 1) *
        // Math.abs(brake))) * inputSpeed);
        return 0.0;
    }

    /**
     * A function to modify the joystick values using linear interpolation. The
     * objective is augment the joystick value going to the motor controllers to
     * widen the region of "fine" control while still allowing full speed.
     *
     * @param input
    ...
    private double interpolate(double input) {
        // TODO: Modify this to take in the set of points as a parameter, then
        // move out to utils package.
        double retVal = 0.0;
        boolean done = false;
        double m, b;
        ...
        for (int i = 1; !done && i < joystickScale.length; i++) {
            if (input >= joystickScale[i][0]) {
                ...
                m = Util.slope(joystickScale[i - 1][0],
                        joystickScale[i - 1][1], joystickScale[i][0],
                        joystickScale[i][1]);
    ...
    static double joystickScale[][] = {
        ...
        { 1.00, 1.00 }, { 0.90, 0.68 }, { 0.06, 0.2 }, // minSpeed
        { 0.06, 0.00 }, { 0.00, 0.00 }, { -0.06, 0.00 }, { -0.06, -0.2 }, // minSpeed
        { -0.90, -0.68 }, { -1.00, -1.00 } };
}
