...
public class ConsolePrinter {
	...
	public void startThread() {
		...
		try {
			...
			log.println("BNOX \t BNOY \t BNOZ \t GYRO SPI");
 			log.flush();
	...
	public void print() {
		if (RobotMap.PRINT_SD_DEBUG_DATA) {
			...
			position = Robot.drivetrain.bno.getVector();
			
			SmartDashboard.putNumber("BNOX", position[0]);
			SmartDashboard.putNumber("BNOY", position[1]);
			SmartDashboard.putNumber("BNOZ", position[2]);
